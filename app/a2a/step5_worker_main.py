"""Container entrypoint for the Step 5 Candidate Context A2A worker (Turn E).

Reads the advertised AgentCard URL from Settings (``STEP5_WORKER_URL``) and the
bind host/port from the environment, then hands off to the EXISTING
``run_step5_worker`` — which builds the worker from ``app.deps`` and serves it via
...
``CandidateContextAgent`` directly, never bypasses ``python_a2a.A2AServer``, and
...
from .step5_worker import run_step5_worker


def main() -> None:
    settings = get_settings()
    run_step5_worker(
        url=settings.step5_worker_url,
        host=os.environ.get("WORKER_BIND_HOST", "0.0.0.0"),
        port=int(os.environ.get("WORKER_BIND_PORT", "8005")),
    )
