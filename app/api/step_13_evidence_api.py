"""Step 13 API — scientific_evidence_table (EvidenceAgent)."""
...
router = APIRouter(prefix="/runs/{run_id}/steps/13", tags=["step-13-evidence"])
...
@router.post("/execute")
def execute_step_13(run_id: str) -> dict:
    storage = get_storage()
    decision = execution_decision(_load_plan(storage, run_id), "step_13_evidence")
    if not decision.allow:
        raise WorkflowStateError(
            "Step 13 cannot execute under current Step 4 plan",
            detail={
