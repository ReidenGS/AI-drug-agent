...
router = APIRouter(prefix="/runs/{run_id}/steps/12", tags=["step-12-ranking"])
...
@router.post("/execute")
def execute_step_12(run_id: str) -> dict:
    storage = get_storage()
    decision = execution_decision(_load_plan(storage, run_id), "step_12_ranking")
    if not decision.allow:
        raise WorkflowStateError(
            "Step 12 cannot execute under current Step 4 plan",
            detail={
                "step_id": "step_12_ranking",
    ...
    return svc.build_ranking_table(run_id).model_dump()
