"""Step 18 API — redesign / optimization trigger scaffold."""
...
router = APIRouter(prefix="/runs/{run_id}/steps/18", tags=["step-18-redesign-trigger"])
...
@router.post("/execute")
def execute_step_18(run_id: str) -> dict:
    storage = get_storage()
    decision = execution_decision(_load_plan(storage, run_id), "step_18_redesign_trigger")
    if not decision.allow:
        raise WorkflowStateError(
            "Step 18 cannot execute under current Step 4 plan",
            detail={
                "step_id": "step_18_redesign_trigger",
                "plan_status": decision.plan_status,
                "planned_status": decision.planned_status,
                "reason": decision.reason,
            },
        )
    svc = RedesignTriggerService(
        storage=storage,
        registry=get_registry_service(),
        workflow_state=get_workflow_state_service(),
    )
    return svc.build_tasks(run_id).model_dump()
